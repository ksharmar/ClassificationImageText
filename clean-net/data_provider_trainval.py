...
#         else:
        ...
        if self.i + batch_size < self.data_size:
            ...
            # this code will make it cyclic (KS)
            ...
            query_samples = self.query_data[self.i:self.data_size,:]
            self.reset()
        ...
        batch_vflag = np.ones([batch_query.shape[0]])

        # get corresponding reference set
        batch_ref = []
        for i in range(batch_query.shape[0]):
