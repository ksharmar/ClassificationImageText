    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        while cur_step < opt.n_step:
            ...
            if cur_step % opt.val_interval == 0 and cur_step != 0:
                ...
                save_path = saver.save(sess, opt.checkpoint_dir + 'model', global_step=cur_step)
                print("Model saved in path: %s" % save_path)
                if avg_val_acc > best_avg_val_acc:
                    # savepath=../trained_models/hymenoptera_data/results/checkpoints/model-200
                    # checkpoint file in checkpoints dir=../trained_models/hymenoptera_data/results/checkpoints/checkpoint
                    best_avg_val_acc = avg_val_acc
                    model_path = os.path.join(opt.checkpoint_dir, "checkpoint")
                    best_model_path = os.path.join(opt.checkpoint_dir, "best_model_{}".format(cur_step))
                    shutil.copy(model_path, best_model_path)
                    print("Best model saved in path: %s" % best_model_path)
                    # however inference restoring still happens from latest in checkpoint file (not best model file)
